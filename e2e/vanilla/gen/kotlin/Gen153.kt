...
object Gen153 {
  fun transform(xs: List<Int>): Map<String, List<Int>> =
    xs.groupBy { "bucket${it % 6}" }.mapValues { (_, v) -> v.sorted() }

  fun pipeline(n: Int): List<Pair<Int, String>> =
    (0 until n).map { it to "v153_$it" }.filter { it.first % 2 == 0 }
  ...
  fun seed(): List<Int> = generateSequence(153) { it + 1 }.take(14).toList()
}
